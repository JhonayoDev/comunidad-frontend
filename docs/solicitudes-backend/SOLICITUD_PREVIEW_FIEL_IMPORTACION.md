# Solicitud BE: Preview fiel con datos completos

> Para que el preview de planilla sea 100% fiel (tipo unidades fase 5 / meta) también para .xlsx
> Nota: el número de versión de migración/cambio lo define el equipo backend (aquí no se presume versión).
...
## Frontend
* `src/composables/usePlanillaDatos.js:previewFilasRaw` + `PlanillaStagedPreview.vue:hasPreviewFiel` ya muestran tabla fiel cuando hay datos completos; cuando el backend devuelva el detalle completo se podrá eliminar el parse local y usar solo backend.
