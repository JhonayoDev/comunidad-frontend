  ...
  async (v) => {
    if (v && props.encomiendaId) {
      ...
      try {
        ...
        const { data } = await encomiendasService.getEncomienda(
          cid,
          props.encomiendaId,
        );
...
<template>
  <Dialog
    ...
    :header="
      encomienda ? `Encomienda — Casa ${encomienda.unidadNumero}` : 'Detalle'
    "
    modal
    :closable="!loading"
    ...
    <div v-else-if="encomienda" class="flex flex-col gap-3">
      ...
      >
        ...
        <div class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-sm">
          ...
          <Tag
            :value="estadoLabel(encomienda.estado)"
            :severity="estadoSeverity(encomienda.estado)"
            size="small"
          />

          <span v-if="encomienda.accesoNombre" class="text-text-muted"
            >Recepción:</span
          >
          <span v-if="encomienda.accesoNombre">{{
            encomienda.accesoNombre
          }}</span>
          ...
          <span v-if="encomienda.nombreRetira" class="text-text-muted"
            >Retirada por:</span
          >
          <span v-if="encomienda.nombreRetira">
            {{ encomienda.nombreRetira }}
            <span v-if="encomienda.rutRetira"
              >({{ encomienda.rutRetira }})</span
            >
          </span>
        </div>

        <div
          v-if="encomienda.observaciones"
          class="border-t border-border/60 pt-2 text-sm"
        >
      ...
      <div
        v-if="encomienda.historial?.length"
        class="bg-background/90 p-3 flex flex-col gap-2 rounded-lg border border-border/60 shadow-lg"
      >
        <p class="text-sm font-semibold m-0">Historial</p>
        <div
          v-for="h in encomienda.historial"
          :key="h.id"
