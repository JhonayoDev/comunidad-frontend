...
const { pasos, configuraciónCompleta, sincronizarTotales } =
...
async function cargar() {
  ...
  try {
    ...
    sincronizarTotales(data?.totales);
  } catch (e) {
...
<template>
  <div class="p-4 flex flex-col gap-4">
    ...
    <template v-if="loading">
      <Card
        ><template #content><Skeleton width="60%" height="1.5rem" /></template
      ></Card>
    ...
    <template v-else-if="dashboard">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-xl font-bold m-0">
                {{ dashboard.condominio?.nombre || "Dashboard" }}
              </h1>
              <span
      ...
      <Card
        v-if="!configuraciónCompleta && pasosPendientes.length"
        class="w-full"
      >
        <template #content>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <Card>
          ...
          >
            <i
              class="pi pi-megaphone text-primary"
      ...
      <Card>
        ...
        <template #content>
          ...
          <div v-else class="flex flex-col gap-2">
            <div
              v-for="m in dashboard.accesos.ultimosMovimientos"
              :key="m.id"
              class="flex items-center justify-between p-2 surface-ground border-round"
            >
              <div class="flex items-center gap-2 min-w-0">
                <Avatar
                  :label="(m.nombreVisitante || '?')[0]"
                  size="small"
                  shape="circle"
                  class="shrink-0"
                />
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="w-full bg-surface-200 h-2 border-round overflow-hidden">
              <div
                ...
                :style="{
                  width:
                    (dashboard.gastoComunActual.porcentajePagado || 0) + '%',
                }"
              ></div>
            </div>
            <div class="flex justify-between text-xs text-surface-400">
              ...
              <span
                >${{
                  Number(
                    dashboard.gastoComunActual.montoRecaudado || 0,
                  ).toLocaleString("es-CL")
                }}
                / ${{
                  Number(
                    dashboard.gastoComunActual.montoEsperado || 0,
                  ).toLocaleString("es-CL")
                }}</span
              >
            </div>
            <Tag
              v-if="dashboard.gastoComunActual.estado === 'CERRADO'"
              value="Cerrado"
              severity="warn"
              class="mt-1"
            />
