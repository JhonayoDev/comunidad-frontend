...
async function cargar() {
  ...
  try {
    ...
    // Por fila GET /configuracion-almacenamiento (404 → Sin configurar)
...
async function guardar() {
  ...
  try {
    ...
    const { data } = await almacenamientoService.guardar(
      editandoId.value,
      payload,
    );
...
<template>
  <div class="p-4 flex flex-col gap-4">
    ...
    <p class="text-sm text-text/95 m-0">
      Configuración por condominio (<code
        >GET/PUT /condominios/{id}/configuracion-almacenamiento</code
      >
      con <code>ALMACENAMIENTO_CONFIGURAR</code>). 404 → Sin configurar.
      SUPER_ADMIN tiene permiso vía <code>rol_permisos</code>.
    </p>
              ...
              <tr v-for="c in condominios" :key="c.id">
                ...
                <td class="whitespace-nowrap">
                  ...
                  <template v-else-if="cfgDe(c.id).data">
                    <Tag
                      :value="cfgDe(c.id).data.proveedor"
                      severity="info"
                      size="small"
                    />
                  </template>
                ...
                <td class="whitespace-nowrap">
                  <Tag
                    v-if="cfgDe(c.id).data"
                    :value="cfgDe(c.id).data.activa ? 'Sí' : 'No'"
                    :severity="
                      cfgDe(c.id).data.activa ? 'success' : 'secondary'
                    "
                    size="small"
                  />
                ...
                <td class="whitespace-nowrap">
                  <template v-if="cfgDe(c.id).data">
                    <Tag
                      v-if="
                        cfgDe(c.id).data.hasSecretKey ||
                        cfgDe(c.id).data.hasDriveCredentials
                      "
                      value="Configurada"
                      severity="success"
                      size="small"
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="flex flex-wrap gap-1">
                  ...
                  <template v-else-if="cfgDe(c.id).data">
                    ...
                    />
                    <Tag
                      :value="cfgDe(c.id).data.activa ? 'Activa' : 'Inactiva'"
                      :severity="
                        cfgDe(c.id).data.activa ? 'success' : 'secondary'
                      "
                      size="small"
    ...
    >
      <div class="flex flex-col gap-4">
        <Message v-if="mensaje" severity="error" :closable="false">{{
          mensaje
        }}</Message>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div class="flex flex-col gap-1">
              ...
              <InputText
                ...
                type="password"
                placeholder="Vacío mantiene actual"
              />
              <small
